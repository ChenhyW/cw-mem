...
(async () => {
  ...
  const respBody = { sessionId: session_id, promptId: prompt_id, response: response };
  const r = await post('/api/prompts/response', respBody);
  log.info('STOP response ' + (r.ok ? 'recorded changes=' + (r.changes!=null?r.changes:'-') : 'write FAILED') + ': session=' + session_id);
  if (!r.ok) {
    remember('/api/prompts/response', respBody);
  } else {
    const sumBody = { sessionId: session_id, promptId: prompt_id };
    const s = await post('/api/prompts/summarize', sumBody);
    log.debug('POST /api/prompts/summarize: ok=' + s.ok + ', body=' + (s.body||''));
    if (!s.ok) remember('/api/prompts/summarize', sumBody);
  }
  suppress();

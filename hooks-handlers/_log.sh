#!/usr/bin/env bash
# memory-lite: shared logging module (bash side)
#
# 用法: source 本文件后调用
#   log_debug "msg"
#   log_info  "msg"
...
#
# 前置: 调用方应先设置 MEMORY_LITE_DATA_DIR (例如 ~/.memory-lite)。

MEMORY_LITE_LOG_LEVEL="${MEMORY_LITE_LOG_LEVEL:-}"
...
_log_today_file() {
  local d
  d=$(date '+%Y%m%d')
  echo "$MEMORY_LITE_DATA_DIR/memory-lite-$d.log"
}

_log_load_config() {
  # 把 config.json 的三项塞进全局变量 _CFG_*
  _CFG_LEVEL="info"; _CFG_RETENTION=3; _CFG_PREVIEW=40
  if [ -f "$MEMORY_LITE_DATA_DIR/config.json" ]; then
    local cfg
    cfg=$(node -e "
...
" "$MEMORY_LITE_DATA_DIR/config.json" 2>/dev/null) || cfg="info 3 40"
...
_log_resolve_level() {
  ...
  if [ -z "$LOG_LEVEL_RESOLVED" ]; then
    if [ -n "$MEMORY_LITE_LOG_LEVEL" ]; then
      LOG_LEVEL_RESOLVED="$MEMORY_LITE_LOG_LEVEL"
    elif [ -f "$MEMORY_LITE_DATA_DIR/loglevel" ]; then
      LOG_LEVEL_RESOLVED="$(tr -d '[:space:]' < "$MEMORY_LITE_DATA_DIR/loglevel")"
    else
      _log_load_config
...
_log_write() {
  ...
  mkdir -p "$MEMORY_LITE_DATA_DIR" 2>/dev/null
...
log_cleanup() {
  ...
  for f in "$MEMORY_LITE_DATA_DIR"/memory-lite-*.log; do
    [ -e "$f" ] || continue

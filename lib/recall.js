...
//
// 相似度: sqlite-vec 返回 L2 距离(非平方), 写入时已 L2 归一化, 故 sim = 1/(1+distance)
// 与 cosine 同向单调。归一化向量下 sim ∈ [0.4142, 1.0]: 0.5↔cos0.50, 0.7↔cos0.908。
// 详见 lib/vector.js 顶部的换算说明。
...
// 返回 { text, hits, error, minScore, candidates, maxSim }
//   hits        供审计/UI 展示
//   minScore    本次实际生效的注入阈值(配置缺省时是代码兜底值, 不是配置原值)
//   candidates  KNN 返回的候选条数(过滤阈值与 skip 之前)
//   maxSim      最相似候选的 sim; 无候选时 null
...
async function userPromptInjection({ db, embedFn, cfg, project, prompt }) {
  ...
  const diag = { minScore: minScore, candidates: 0, maxSim: null };
  ...
  if (rows.length) diag.maxSim = 1 / (1 + (rows[0].distance || 0));

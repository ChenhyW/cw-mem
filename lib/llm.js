// lib/llm.js — cw-mem LLM 客户端 + 3 套 prompt 模板 + observation 校验。
//
...
// tool 批量观察: 一组连续同类工具调用 → 1 条合并 observation(JSON)
const TOOL_BATCH_TEMPLATE = `[工具批量] 共 {count} 次连续 {tool_name} 调用:
{calls}

请把这一组连续同类工具调用合并提炼为一条结构化观察(覆盖整组净效果, 而非逐条罗列)。只输出一个合法 JSON 对象, 不要代码块、不要额外说明, 字段固定如下:
{{
  "title": "一句话标题, 这组调用让系统/项目变得不同了什么",
  "action": "这组调用整体要完成什么",
  "type": "bugfix|feature|refactor|change|discovery|decision|security|skip 之一",
  "concepts": ["how-it-works|why-it-exists|what-changed|problem-solution|gotcha|pattern|trade-off 等, 1-5 个"],
  "filesChanged": ["整组涉及的全部持久修改文件路径(去重), 无则空数组"],
  "result": "整组的输出/效果",
  "sideEffect": "环境/网络等非文件副作用, 无则空字符串"
}}`;

const TOOL_BATCH_SYSTEM = `你是一位工具调用观察助手, 负责把一组连续的同类工具调用合并提炼成一条可注入长期记忆的结构化观察。

合并原则:
- 重点关注整组净效果(系统/项目变得不同了什么), 而非逐条复述每个调用。
- filesChanged 汇总整组涉及的文件(去重)。
- 若整组都是纯读取/无实质效果, 判为 skip(仍照写 JSON 保留审计)。
- 用动词: implemented / fixed / refactored / migrated / added / configured / discovered / confirmed。

type 选择规则同单条: bugfix/feature/refactor/change/discovery/decision/security/skip。
严格输出合法 JSON; 无信息填空字符串或空数组, 不要填 "无"。`;
...
const TEMPLATES = {
  ...
  tool_batch: { user: TOOL_BATCH_TEMPLATE,       system: TOOL_BATCH_SYSTEM },

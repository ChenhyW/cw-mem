...
const { runStopBatch, runSessionSummary, runVectorRetry, findSessionsNeedingSummary } = require('./batch');
...
function startServer({ dataDir, uiDir, port }) {
  return new Promise((resolve, reject) => {
    ...
    const state = { db, cfg, dataDir, uiDir, port: null, server: null, summaryRetryTimer: null, summaryBusy: false, vectorBusy: false, sessionSummaryBusy: false };
    ...
    function startSummaryRetryTimer() {
      ...
      state.summaryRetryTimer = setInterval(() => { runSummaryRetryRound(); runSessionSummaryRound(); runVectorRetryRound(); }, intervalSec * 1000);
    ...
    // 会话摘要结果落日志(此前 SessionEnd 处 .catch(() => {}) 静默吞掉, 失败无从排查)
    function logSessionSummaryResult(sessionId, r) {
      if (r.status === 'success') {
        console.log('[cw-mem] session 摘要完成 session=' + sessionId + ' id=' + r.id + (r.vectorError ? ' vector=' + r.vectorError : ''));
      } else if (r.status === 'skipped') {
        // eslint-disable-next-line no-console
        console.log('[cw-mem] session 摘要跳过 session=' + sessionId + ' reason=' + r.reason);
      } else {
        // eslint-disable-next-line no-console
        console.warn('[cw-mem] session 摘要失败 session=' + sessionId + ': ' + (r.error || r.reason || r.status));
      }
    }

    // 一轮会话摘要补齐: SessionEnd 时 result 摘要常还没落库(runSessionSummary 返回 skipped),
    // 于是"退出即无摘要"。这里按 retryInterval 周期找"已 ended 但尚无摘要"的会话重跑, 收敛后自然停止。
    function runSessionSummaryRound() {
      if (state.sessionSummaryBusy) return;
      state.sessionSummaryBusy = true;
      let ids = [];
      try { ids = findSessionsNeedingSummary(db, 3); }
      catch (e) {
        // eslint-disable-next-line no-console
        console.error('[cw-mem] session summary 扫描异常: ' + e.message);
        state.sessionSummaryBusy = false;
        return;
      }
      const jobs = ids.map(id => runSessionSummary({ db, cfg: reloadCfg(), sessionId: id, llmMod: llm, embedFn })
        .then(r => logSessionSummaryResult(id, r))
        .catch(e => {
          // eslint-disable-next-line no-console
          console.warn('[cw-mem] session 摘要失败 session=' + id + ': ' + (e && e.message || e));
        }));
      if (jobs.length === 0) { state.sessionSummaryBusy = false; return; }
      Promise.all(jobs).finally(() => { state.sessionSummaryBusy = false; });
    }
        ...
        if (pathname === '/api/sessions/summarize' && req.method === 'POST') {
          return readBody(req, (err, data) => {
            ...
            runSessionSummary({ db, cfg: reloadCfg(), sessionId, llmMod: llm, embedFn })
              .then(r => logSessionSummaryResult(sessionId, r))
              .catch(e => {
                // eslint-disable-next-line no-console
                console.warn('[cw-mem] session 摘要失败 session=' + sessionId + ': ' + (e && e.message || e));
              });

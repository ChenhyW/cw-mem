...
function startServer({ dataDir, uiDir, port }) {
  return new Promise((resolve, reject) => {
    ...
    function ensureSessionProject(sessionId, candidate) {
      if (!sessionId || !candidate) return candidate || null;
      ...
      db.prepare('INSERT OR IGNORE INTO sessions (id, project_dir, started_at, last_seen_at) VALUES (?, ?, ?, ?)')
        .run(sessionId, candidate, nowIso(), nowIso());
    ...
    function handleSession(data) {
      ...
      ensureSessionProject(sid, data.projectDir || null);
    ...
    function handlePrompt(data) {
      ...
      // 落库目录归一化, 不用 hook 的原始 cwd
      const projDir = ensureSessionProject(data.sessionId || '', data.projectDir || null);
      ...
      ).run(data.sessionId || 'unknown', data.prompt || '', type, toolName, projDir, claudePromptId, toolUseId, toolTarget, nowIso());
    ...
    // parent_seq 必须与 UI 的编号口径一致, 否则"归属提示词 #N"和父卡页脚的 #N 对不上。
    // UI 的 buildPromptMaps 用**跨会话全局序号**(PROMPT 行按 created_at,id 升序的 i+1),
    // 不是会话内轮次 —— 按会话内编号会给多个会话都产出 #1/#2, 用户没法据此定位到那张父卡。
    // 故这里也按全局口径: "严格排在它前面的 PROMPT 行数 + 1"。id < 而非 id <=, 否则把自己数进去。
    ...
    // 2) 父行必须早于该工具行。同一 id 可能横跨多个 turn, 不加时间约束时真实库有 3 条卡片
    //    的父行晚于工具行本身。
    // 两条约束下覆盖率 1240/1251(99%), 且无一条时间倒挂; 匹配不到就返回 null, 不做猜测。
    const parentStmt = db.prepare(
      "SELECT id, created_at FROM prompts WHERE type = 'PROMPT' AND session_id = ? AND claude_prompt_id = ?" +
      " AND prompt NOT LIKE '<task-notification>%' AND (created_at < ? OR (created_at = ? AND id < ?))" +
      " ORDER BY created_at DESC, id DESC LIMIT 1");
    const seqStmt = db.prepare(
      "SELECT COUNT(*) + 1 AS seq FROM prompts q WHERE q.type = 'PROMPT'" +
    ...
    function attachParentAttribution(rows) {
      for (const r of rows) {
        ...
        r.parent_id = par.id;
        r.parent_seq = seqStmt.get(par.created_at, par.created_at, par.id).seq;
      }
      return rows;
    }

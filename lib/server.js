...
function startServer({ dataDir, uiDir, port }) {
  return new Promise((resolve, reject) => {
    ...
    const parentStmt = db.prepare(
      "SELECT id, created_at FROM prompts WHERE type = 'PROMPT' AND session_id = ? AND claude_prompt_id = ? ORDER BY id DESC LIMIT 1");
    ...
    function attachParentAttribution(rows) {
      for (const r of rows) {
        ...
        const par = parentStmt.get(r.session_id, r.claude_prompt_id);

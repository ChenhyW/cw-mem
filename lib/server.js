...
const { runStopBatch, runSessionSummary } = require('./batch');
...
function startServer({ dataDir, uiDir, port }) {
  return new Promise((resolve, reject) => {
    ...
    const state = { db, cfg, dataDir, uiDir, port: null, server: null, summaryRetryTimer: null, summaryBusy: false };
    ...
    function startSummaryRetryTimer() {
      ...
      state.summaryRetryTimer = setInterval(() => { runSummaryRetryRound(); }, intervalSec * 1000);
    ...
    function runSummaryRetryRound() {
      ...
      try {
        ...
        console.error('[memory-lite] summary retry round error: ' + e.message);
      } finally {

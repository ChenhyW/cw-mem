// lib/db.js — memory-lite SQLite + sqlite-vec 初始化、迁移、连接封装。
//
// 表清单:
...
function openDb(dataDir, embedDim) {
  ...
  const dbFile = path.join(dataDir, 'memory-lite.db');
  ...
  try {
    ...
    if (!_vecWarned) {
      ...
      console.warn('[memory-lite] sqlite-vec 扩展加载失败: ' + e.message + ' (向量化相关功能将不可用)');
    }
  }
...
function _migrate(db, embedDim) {
  ...
  if (!_tableExists(db, 'memories_vec')) {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn('[memory-lite] memories_vec 维度不匹配, 需在 UI 改 ollama.embedDim 后手动 DROP 或重启清理: ' + e.message);
    }
  }
}

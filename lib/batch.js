...
async function runToolGroupSummary({ db, cfg, toolRowIds, llmMod, embedFn }) {
  ...
  try {
    ...
    obs.count = rows.length;   // 组内调用数, 供 UI 显示"N 次合并"徽标; 校验器白名单外, 故在 validate 之后补
...
async function runStopBatch({ db, cfg, promptRowId, llmMod, embedFn }) {
  ...
  try {
    // ── 1. tool 观察(只读, 不调 LLM) ──
    // tool 摘要由 Consumer 的流式聚合(runToolGroupSummary)在 Stop 之前落库,
    // 所以这里读到的一定是已完成的摘要而非空结果。此处不再重做工具摘要,
    // 也就不会再出现"父 PROMPT 重试把已成功的 tool 回退成 failed"的问题。
    ...
    const toolObsList = done.map(r => {
      let m = null;
      try { m = JSON.parse(r.summary_meta); } catch (e) { return null; }   // 损坏行跳过, 不让一条坏数据毁掉 result 摘要
      if (!m) return null;
    ...
    const parsed = _parseJsonSafe(text);
    if (!parsed) {
      // invalid json 是可重试的瞬时故障(LLM 偶尔输出非 JSON), 走行级状态机, 不直接判死刑
      _applyFailure(db, row, 'invalid result summary json', cfg, llmMod);
...
async function runSessionSummary({ db, cfg, sessionId, llmMod, embedFn }) {
  ...
  try {
    ...
    const res = db.prepare(`
      INSERT INTO session_summaries (session_id, request, investigated, learned, completed, next_steps, notes, seq, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(sessionId, parsed.request || '', parsed.investigated || '', parsed.learned || '', parsed.completed || '', parsed.next_steps || '', parsed.notes || '', nextSeq(db), nowIso());

  ...
  try {
    ...
    if (cfg.toolSummary && cfg.toolSummary.enabled) {
      ...
      for (const tr of toolRows) {
        // 已成功的工具观察不重做: 避免父 PROMPT 重试时把 success→generating→(偶发失败)failed_pending_retry 回退
        if (tr.summary_status === 'success') continue;
        ...
        try {
          ...
          // 失败不回退摘要成功状态(摘要已落库), 召回时缺向量自然不命中, 由向量重试兜底
          try {
...
module.exports = { runStopBatch, runSessionSummary, runVectorRetry };

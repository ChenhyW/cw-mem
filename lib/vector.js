...
// (已实测: [1,0] 与 [0,1] 的 distance = √2 = 1.41421, 而非 2)。归一化向量下 L2 = √(2-2·cos),
// 故 **cosine = 1 - d²/2** 就是标准余弦相似度, 直接作为相似度口径与 minCosine 阈值。
// 早期用过 sim = 1/(1+L2) 做阈值代理, 但它把值域压成 [0.4142, 1.0]、与常识里的"余弦相似度"
// 不是一把尺子, 已废弃 —— 现在阈值与 UI 显示统一用余弦。
...
// sqlite-vec 的 distance → cosine。注入阈值、UI 显示、探查端点都必须走这一个函数,
// 否则各处各自手算迟早漂移。
//   cos = 1 - L2²/2, 故 distance=0 ↔ cos=1, distance=√2(正交) ↔ cos=0, distance=2 ↔ cos=-1
...
module.exports = {
  vecToBuffer, normalize, storeEmbedding, deleteEmbedding,
  knnRecall, hybridFileRecall, cosineFromDistance
};

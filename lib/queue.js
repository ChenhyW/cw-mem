...
class Consumer {
  ...
  startFull(handlers, dataDir) {
    ...
    this.recover();
    this.restartSweep(handlers, dataDir);
  }

  // 配置热更新: sweepIntervalSeconds 变更时重启补齐定时器(立即生效, 无需重启 server)
  restartSweep(handlers, dataDir) {
    if (this.#sweepTimer) { clearInterval(this.#sweepTimer); this.#sweepTimer = null; }
  ...
  async #runResult(item) {
    // 消费口兜底: 退避重试或 recover 重建的 item 可能指向已成功的行, 不重复烧 LLM。
    // 只在这里判, 因为 Lane FIFO 已保证同 session 不并发, 竞态面只剩"状态已变"。
    const st = this.#db.prepare('SELECT summary_status FROM prompts WHERE id = ?').get(item.promptRowId);
    if (st && (st.summary_status === 'success' || st.summary_status === 'generating')) return;
  ...
  async #runSession(item) {
    ...
    // 结果落日志: 此前 SessionEnd 处 .catch(() => {}) 静默吞掉, "退出即无摘要"无从排查
    if (r.status === 'success') console.log('[cw-mem] session 摘要完成 session=' + item.sessionId + ' id=' + r.id);
    else if (r.status === 'skipped') console.log('[cw-mem] session 摘要跳过 session=' + item.sessionId + ' reason=' + r.reason);
    else console.warn('[cw-mem] session 摘要失败 session=' + item.sessionId + ': ' + (r.error || r.reason || r.status));

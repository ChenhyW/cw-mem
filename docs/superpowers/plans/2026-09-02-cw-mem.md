# cw-mem Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
>
> **⚠️ 修订记录(2026-09-11):① 相似度口径 `sim = 1/(1+L2)` → 余弦 `cos = 1 - d²/2`,配置键 `minScore` → `minCosine`(默认 0.5);② 修正 sqlite-vec 返回值描述 —— 它返回 L2 距离而非 L2 平方(已实测,见 `lib/vector.js`)。代码片段与相关描述已同步,其余为 2026-09-02 原始计划。**
...
- Data dir: `~/.cw-mem/` (`config.json`, `cw-mem.db`, logs). Env override `CW_MEM_DATA_DIR`.
...
```json
{ "name": "cw-mem", "version": "0.1.0", "author": { "name": "<owner>" }, "description": "轻量级持久化记忆插件" }
```
...
```json
{
  ...
  "plugins": [{ "name": "cw-mem", "version": "0.1.0", "source": ".", "description": "轻量级持久化记忆插件" }]
}
```
...
Copy `/Volumes/KINGSTON-CHENHY/plugins/cw-mem/hooks-handlers/_log.sh` to `hooks-handlers/_log.sh`; replace every `CW_MEM` → `CW_MEM` and `cw-mem-` → `cw-mem-` (log file prefix). Keep behavior identical (level-resolved logging, daily rotation, truncation).

- [ ] **Step 7: Commit**

```bash
git add package.json .claude-plugin/ hooks/ hooks-handlers/_log.sh .gitignore
git commit -m "chore: scaffold cw-mem repo + manifests + hooks.json + log module"
```
...
Port structure from cw-mem `ui/server.js` `DEFAULT_CONFIG`/`loadConfig`/`saveConfig`/`mergeLlm`, with cw-mem schema (spec §9): flat `server`/`log`/`llm`/`ollama`/`toolSummary`/`recall`. `mergeLlm` validates provider∈{openai-compatible,anthropic}, reasoningEffort∈{'',none,low,medium,high}, numeric clamps. `loadConfig(dataDir)` reads `<dataDir>/config.json` merging over `DEFAULT_CONFIG`; `saveConfig` writes it. All field-validators mirror cw-mem's conditional-assign pattern.
...
- `openDb(dataDir, embedDim)`: open `<dataDir>/cw-mem.db` with better-sqlite3, `journal_mode=WAL`; `db.loadExtension(require('sqlite-vec').getLoadablePath())` (try/catch, log warn if unavailable).
...
- `userPromptInjection`: `embedFn({url,model,input:prompt})` → query vector (buffered via `vecToBuffer`); `knnRecall` topK; compute `cos = 1 - distance²/2` (embeddings are L2-normalized at store time and sqlite-vec returns L2 distance, so L2 = √(2-2cos) and this is exact cosine); filter `cos >= minCosine`, `type !== 'skip'`, same project, dedupe by ref_id, slice `injectMaxCount`; `hits = [{entity_type, ref_id, title, cosine}]` (read title from `summary_meta` via ref_id); `text` = `## 相关过往工作` + each hit's title/subtitle + files_modified, char-trimmed to `injectMaxTokens`.
...
- stdout: `{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"<text>"},"systemMessage":"🧠 cw-mem 已生效"}` (if text empty, emit minimal banner only).
...
Temporarily add `log.info("[HOOK] stdin=$RAW_JSON")` to each handler; run a real Claude Code session; send a prompt, use a tool, stop, exit. Confirm `~/.cw-mem/cw-mem-<date>.log` shows the verified stdin fields (spec §2.1) and the `POST /api/...` lines. Remove debug logs after.
...
Copy cw-mem `ui/index.html` structure (header w/ project select + theme + settings; prompt list; settings modal with `取消/保存/保存并重启`); adapt fetch endpoints to cw-mem; retheme brand.
...
Push repo to GitHub; in Claude Code run `/plugin marketplace add <github-repo>` then `/plugin install cw-mem@cw-mem`. Confirm it appears in `~/.claude/plugins/cache/cw-mem/cw-mem/<version>/`.
...
New Claude Code session: confirm SessionStart injection + recalled summaries; send a prompt → UserPromptSubmit injection + card; use a tool → tool card linked; stop → result summary; exit → session summary. Check `~/.cw-mem/` db has rows + vectors.
...
**Distance metric:** sqlite-vec returns L2 distance (not squared — verified: `[1,0]` vs `[0,1]` gives √2 ≈ 1.41421, not 2). Embeddings are L2-normalized at store time (Task 6 `normalize`), so L2 = √(2-2cos) and `cos = 1 - distance²/2` (Task 7) is exact cosine, used directly as the `minCosine` threshold. Non-normalized vectors would break this — normalize-on-store is mandatory.
